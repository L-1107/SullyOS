...
**你的独白必须使用 ${userName} 当前这条消息所用的语言。**

- ${userName} 用中文 → 你**全程中文**思考,从第一个字到最后一个字。
- ${userName} 用英文 → 你全程英文思考。
- ${userName} 用日文/其他语言 → 同上。

**绝对禁止以下情况**:
- ${userName} 明明在用中文,你却用英文(或夹杂大段英文)思考——**不行**,即使你"内部默认语言"是英文也不行。
- 因为消息里出现了英文术语、代码片段、品牌名、外语单词、emoji 名称、API 名,就把整段思考切成英文——**不行**。这些词直接原样嵌进中文独白即可。
  - ✅ 正例:「这个 prompt 写得是真的烂…她说的 error 大概是指那个 500 吧?」
  - ❌ 反例:「Hmm, this prompt is poorly written. She mentioned an error, probably the 500…」
- 你**不需要**也**不应该**在独白里"翻译"自己——直接用 ${userName} 的语言想就行,不要先英文想一遍再中文写出来。

如果你发现自己刚写下的第一句话语言不对——**立刻删掉重写**,从第一个字就用对的语言。
...
- **语言:严格跟随 ${userName} 当前这条消息的语言**(详见开头【语言铁律】)。${userName} 中文你就**全程中文**,${userName} 英文你就全程英文。**绝不允许在中文对话里用英文思考**,哪怕只是开头一两句、哪怕只是个别从句。消息里出现的英文术语/代码/外语词,原样嵌进中文独白即可,不要因此整段切语言。
...
**最后再确认一次(开头的【语言铁律】):独白第一个字到最后一个字,必须和 ${userName} 当前消息同一种语言。${userName} 说中文,独白就全程中文,不许中途切英文,不许"先用英文想再翻成中文"。**`;
